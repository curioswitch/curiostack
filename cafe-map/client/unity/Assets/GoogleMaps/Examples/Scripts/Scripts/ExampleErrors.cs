...
/// <summary>
/// Class for generating and returning specific kinds of error messages that are frequently used in
/// the Maps Unity SDK example scenes.
/// </summary>
public static class ExampleErrors {
  /// <summary>
  /// Returns a <see cref="string"/> error message saying that a needed parameter was not defined on
  ...
  /// message in the Unity console, Unity will open the script that generated the error (the script
  /// calling this <see cref="ExampleErrors"/> class).
  /// </para></remarks>
  /// <param name="script">
  /// The script calling this code (can be added using the alias: <code>this</code>). Assumed to not
  /// be null.
  /// </param>
  ...
  public static string MissingParameter<T>(MonoBehaviour script, T missingParameter,
      string missingParameterName, string context = "to operate") {
    // Note: 'name' and 'GetType()' just give the name of the GameObject this script is on, and
    // the name of this script respectively.
    return string.Format("No {0} defined for {1}.{2}, which needs a {3} {4}.",
        missingParameterName, script.name, script.GetType(), typeof(T), context);
  }

  /// <summary>
  /// Returns a <see cref="string"/> error message saying that an array of parameters needed by this
  /// <see cref="GameObject"/> had no elements in it.
  /// </summary>
  /// <remarks>
  /// This method returns a <see cref="string"/> error message for use with <see cref="Debug"/>.
  /// <para>
  ...
  /// error message, specifically to say that this script needs this parameter [reasonRequired]. By
  /// default this is set to the generic reason 'to operate'.
  /// </param>
  /// <typeparam name="T">
  /// The type of elements contained in <see cref="emptyArray"/>. Used in generating error message.
  /// </typeparam>
  /// <returns>Error message for displaying with <code>Debug.LogError</code>.</returns>
  public static string EmptyArray<T>(MonoBehaviour script, T[] emptyArray,
      string emptyArrayName, string context = "to operate") {
    return string.Format(
        "Empty {0} array defined for {1}.{2}, which needs a non-empty array of {3}s {4}.",
        emptyArrayName, script.name, script.GetType(), typeof(T), context);
  }

  /// <summary>
  /// Returns a <see cref="string"/> error message saying that an array of parameters needed by this
  /// <see cref="GameObject"/> contained at least one null element.
  /// </summary>
  /// <remarks>
  /// This method returns a <see cref="string"/> error message for use with <see cref="Debug"/>.
  /// <para>
  ...
  public static string NullArrayElement<T>(
    ...
    return string.Format("Null {0} defined for {1}.{2}, specifically {3} {4} of {5}).",
        arrayName, script.name, script.GetType(), typeof(T), index, array.Length);
  }
  ...
  public static string InvalidArrayIndex<T>(
      MonoBehaviour script, T[] array, string arrayName, int index) {
    return string.Format("{0} index of {1} given to {2}.{3} for use in {4} array.\nValid indices "
        + "are in the range 0 to {5} based on {6} stored {7}s in {3}'s {4} array.",
        index < 0 ? "Negative" : "Invalid", index, script.name, script.GetType(), arrayName,
        array.Length - 1, array.Length, typeof(T));
  }
  ...
  public static string NullMainCamera(MonoBehaviour script, string context = "to operate") {
    return string.Format("No Camera.main found in the scene.\nPlease make sure at "
        + "least one in-scene Camera is tagged as \"MainCamera\" in order for {0}.{1} {2}.",
        script.name, script.GetType(), context);
  }
  ...
  /// </param>
  /// <returns>Error message for displaying with <code>Debug.LogError</code>.</returns>
  public static string NotGreaterThanZero(MonoBehaviour script, float value, string valueName,
  ...
  /// </param>
  /// <returns>Error message for displaying with <code>Debug.LogError</code>.</returns>
  public static string NotGreaterThan(MonoBehaviour script, float value, string valueName,
      float minimum, string minimumName = null, string context = "to operate") {
    return string.Format("{0} given to {1}.{2}, which needs a {3} greater than {4} {5}.",
        IsZero(value)
            ? string.Concat(valueName, " of zero")
            : string.Format("{0} {1} of {2}",
                IsZero(minimum) ? "Negative" : "Too small", valueName, value),
  ...
  /// <summary>
  /// Return a <see cref="string"/> error message saying a given <see cref="int"/> value that needed
  /// to be within a given range, was not.
  /// </summary>
  /// <remarks>
  /// This method returns a <see cref="string"/> error message for use with <see cref="Debug"/>.
  /// <para>
  ...
  /// </param>
  /// <returns>Error message for displaying with <code>Debug.LogError</code>.</returns>
  public static string OutsideRange(MonoBehaviour script, int value, string valueName,
      int minimum, int maximum, string context = "to operate") {
    return string.Format("{0} {1} of {2} given to {3}.{4}, which needs a valid {1} (within the "
        + "range of {5} to {6}) {7}.",
        value < minimum
            ? minimum == 0 ? "Negative" : "Too small"
            : maximum == 0 ? "Positive" : "Too large",
        valueName, value, script.name, script.GetType(), minimum, maximum, context);
  }
  ...
  /// </param>
  /// <returns>Error message for displaying with <code>Debug.LogError</code>.</returns>
  public static string OutsideRange(MonoBehaviour script, float value, string valueName,
      float minimum, float maximum, string context = "to operate") {
    return string.Format("{0} {1} of {2} given to {3}.{4}, which needs a valid {1} (within the "
        + "range of {5} to {6}) {7}.",
        value < minimum
            ? IsZero(minimum) ? "Negative" : "Too small"
