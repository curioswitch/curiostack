...
#region Parameters
  ...
  [Tooltip("Should this Label keep its starting x-rotation (i.e. should it stay at its current "
...
#region Variables
  ...
  /// controlled by view angle, to allow manual changes in alpha or fading animations to be combined
  /// with this view-dependant alpha logic.
  /// </summary>
  /// <remarks>
  ...
  /// <see cref="Label.CanvasGroup"/> to be used at any time without having to check if it is null.
  /// </summary>
...
#region Input
  ...
  internal void SetText(string text) {
    ...
    if (!CanFindText()) {
      ...
      Debug.LogErrorFormat("No Text element set for {0}.{1}, which requires a UI.Text element to "
        + "show given text \"{2}\".",
        name, GetType(), text);
      return;
    }
...
#region Coroutines
  ...
  /// <remarks>
  /// If <see cref="FadeWithView"/> is enabled, this <see cref="Label"/> will also fade in or out as
  /// <see cref="Camera.main"/> looks towards or away from it.
  /// </remarks>
  private IEnumerator TrackCamera() {
    ...
    // Start facing Camera.
    while (true) {
      // Get the current rotation of the Camera. If Labels are meant to turn only (y-rotation only,
      // ignoring x-rotation) then remove the x-component of this rotation.
...
#region Private
  ...
  private void StartFading(bool fadeIn) {
    ...
    // Start this Label fading (unless already fading). Note that if already fading, then the change
    // in variables above will set up the new fade with the existing coroutine.
    if (!IsFading) {
      StartCoroutine(Fade());
    }
  ...
  /// over all in-scene <see cref="Label"/>s, calling <see cref="StartFading"/> on each.
  /// </remarks>
  /// <params name="fadeIn">True to start fading in, false to start fading out.</params>
  private static void StartFadingAll(bool fadeIn) {
    // Print an error if there are no Labels to start fading.
    if (AllLabels.Count == 0) {
      Debug.LogErrorFormat("No {0} found in the current scene, so cannot start fading them all "
          + "{1}.",
          typeof(Label), fadeIn ? "in" : "out");
      return;
    }
  ...
  ///<summary>
  /// Set desired alpha level. If alpha is controlled by view angle, apply alpha through the use of
  /// a multiplier, otherwise apply alpha directly to <see cref="CanvasGroup"/> now.
  /// </summary>
  ...
  private void Scale(float multiplier) {
    // Make sure this label is setup, then half/double it's scale.
    TrySetup();
  ...
  /// iterates over all in-scene <see cref="Label"/>s, calling <see cref="HideOrShowText"/> on each.
  /// </remarks>
