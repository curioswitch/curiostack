...
#region Parameters
  ...
  [Tooltip("Time in seconds to animate slider moving up and down. Animation continues until input "
      + "is received from the player.")]
  public float AnimationTime = 6f;

  [Tooltip("Apply smoothing to ends of slider animation (true) or leave ends as linear/sharp "
      + "(false)?")]
...
#region Variables
  ...
  /// <summary>
  /// Has 'no action defined' error message been debugged yet? This flag is used to prevent printing
  /// an error every single frame <see cref="Slider"/>'s value is changed (printing an error on the
  /// first frame only).
  /// </summary>
  ...
  /// <summary>
  /// Flag used to make sure <see cref="Slider"/> is currently approaching <see cref="StopValue"/>.
  /// </summary>
  /// <remarks>
  /// This is used to make sure the <see cref="Slider"/> does not instantly stop if receives a
  /// <see cref="StopValue"/> that is less than the current value. Instead, the <see cref="Slider"/>
  /// must be less than the <see cref="StopValue"/> for at least one frame before can be considered
  /// to have passed the <see cref="StopValue"/>, and trigger a stop.
  /// </remarks>
...
#region Input
  ...
  internal void StartAnimatingAt(float startValue) {
    ...
    if (startValue < 0f || startValue > 1f) {
      Debug.LogErrorFormat("Invalid value of {0:N2} given to {1}.{2}.StartAnimatingAt function\n"
          + "Valid values are within the range 0f to 1f inclusive.\nDefaulting to 0f.",
          startValue, name, GetType());
      startValue = 0f;
    }
  ...
  internal void StopAnimatingAt(float stopValue) {
    ...
    if (stopValue < 0f || stopValue > 1f) {
      Debug.LogErrorFormat("Invalid value of {0:N2} given to {1}.{2}.StopAnimatingAt function.\n"
          + "Valid values are within the range 0f to 1f inclusive.\nDefaulting to 1f.",
        stopValue, name, GetType());
      stopValue = 1f;
    }
...
#region Private
  ...
  private IEnumerator Animate() {
    // Flag that animation is now in progress (prevents this Coroutine being redundant started while
    ...
    while (IsAnimating && !Input.anyKey) {
      ...
      if (BackAndForth) {
        if (AnimationCycle >= 1f) {
          ...
          // When between top and bottom, optionally apply smoothing to ease the transition between
          // going up and down (and vice versa).
        ...
        // how this is measured is based on the direction the slider is currently travelling (up or
        // down).
        animationPercent = travellingUp
            ? TryStopIfAbove(animationPercent) : TryStopIfBelow(animationPercent);
      } else {
  ...
  /// <summary>
  /// See if should stop animation this frame, adjusting given value if have just past stop value by
  /// going downwards, leaving value as is otherwise.
  /// </summary>
  ...
  private float TryStopIfAbove(float value) {
    ...
    if (StopValue.HasValue) {
      ...
      // prevents the animation from stopping immediately if a Stop Value is given that is less than
      // the current value, waiting instead until the moment the value passes the Stop Value before
  ...
  /// <summary>
  /// See if should stop animation this frame, adjusting given value if have just past stop value by
  /// going upwards, leaving value as is otherwise.
  /// </summary>
  ...
  private void TryOnChange(float value) {
    ...
    if (OnChange == null) {
      ...
      if (!NoOnSliderErrorShown) {
        Debug.LogErrorFormat("No OnSlider Action set for {0}.{1}, so cannot inform other classes "
