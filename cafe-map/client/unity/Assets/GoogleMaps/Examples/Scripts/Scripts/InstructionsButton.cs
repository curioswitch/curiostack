...
public sealed class InstructionsButton : MonoBehaviour {
  [Tooltip("Optional Slider to extend upwards to fill the empty space left by this Button when it "
      + "is hidden.")]
  ...
  /// </summary>
  private void Start () {
    // Get required Button, and connect to hiding of this GameObject (and optionally extending given
    // Slider).
    ...
    button.onClick.AddListener(() => {
      ...
      if (Slider != null) {
        // Adjust Slider's height by the height of this Button's displayed image, so that as we hide
        // the Button, the Slider's height is extended over the empty space left by the Button.
