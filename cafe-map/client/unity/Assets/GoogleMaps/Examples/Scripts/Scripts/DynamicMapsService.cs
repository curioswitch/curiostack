...
public sealed class DynamicMapsService : MonoBehaviour {
  ...
  [Tooltip("Maximum distance to render to (prevents loading massive amount of geometry if looking"
      + "up at the horizon).")]
  public float MaxDistance = 1000f;

  [Tooltip("The ground plane. We keep this centered underneath Camera.main, so as we move around "
      + "the game world the ground plane stays always underneath us. As such the Material applied "
      + "to this ground plane should either be untextured, or textured using worldspace "
  ...
  /// <summary>The <see cref="GameObjectOptions"/> to use when rendering loaded geometry.</summary>
  /// <remarks>
  /// This value must be overriden before this script's <see cref="Start"/> function is called in
  /// order to render loaded geometry with a different set of <see cref="GameObjectOptions"/>. If
  /// no change is made, <see cref="ExampleDefaults.DefaultGameObjectOptions"/> will be used
  /// instead.
  /// </remarks>
  ...
  /// <para>
  /// This allows the world to be periodically recentered back to the origin, which avoids geometry
  /// being created with increasingly large floating point coordinates, ultimately resulting in
  /// floating point rounding errors.
  /// </para></summary>
  /// <param name="camera">
  /// <see cref="Camera"/> to use to recenter world.
  /// <para>
  /// This <see cref="Camera"/> will be moved until it is over the origin (0f, 0f, 0f). At the same
  /// time all geometry created by the <see cref="MapsService"/> will be moved the same amount. The
  /// end result is that the world is recentered over the origin, with the change being
  ...
  internal Vector3 RecenterWorld(Camera camera) {
    ...
    // Camera itself, so that the world and the Camera can all be moved until the Camera is over the
    // origin again. Note that the MoveFloatingOrigin function automatically moves all loaded
  ...
  private void RefreshView() {
    // Store the rotation of the camera, so we can check next frame if the main Camera has rotated,
    // and thus if the visible world should be refreshed again.
  ...
  private IEnumerator UnloadUnseen() {
    while(true) {
      ...
      // This is to avoid unloading geometry that may be reloaded again very shortly (as it is right
