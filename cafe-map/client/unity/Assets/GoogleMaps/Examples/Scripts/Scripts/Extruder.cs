...
/// <summary>
/// Static class containing logic for creating extrusions and lofts from given geometry built by the
/// Maps Unity SDK.
/// </summary>
public static class Extruder {
  /// <summary>Default thickness of created extrusions.</summary>
  /// <remarks>This value will be used for all extrusions unless a custom value is given.</remarks>
  ...
  /// information. The chosen value of 10f matches the default value used inside the Maps Unity SDK
  /// for buildings without stored heights.
  /// <para>
  ...
  /// </summary>
  /// <remarks>
  /// Each entry defines the cross section of a parapet in a coordinate space relative to the outer
  /// roof-edge of the building, where the positive x-axis points out away from the building, and
  /// the positive y-axis points towards the sky. So, for example:
  /// <para>(1, 0) is 1 meter out of the building.</para>
  /// <para>(1, -1) is 1 meter out, 1 meter down.</para>
  /// <para>Outlines should be specified with an counterclockwise winding order (assuming +x right,
  /// +y up) to ensure the normals of the generated geometry face in the correct direction.</para>
  /// </remarks>
  ...
  /// One <see cref="GameObject"/> will be returned for each part of the given building.
  /// </para></returns>
  public static GameObject[] AddRandomBuildingParapet(GameObject buildingGameObject,
  ...
  /// The Maps Unity SDK created <see cref="GameObject"/> containing this area.
  /// </param>
  /// <param name="extrusionMaterial">
  ...
  /// </para></returns>
  public static GameObject[] AddAreaOutline(GameObject areaObject,
      Material extrusionMaterial, Area area, float outlineWidth) {
    return ExtrudeEdgeSequences(areaObject, extrusionMaterial,
        area.GenerateBoundaryEdges(), outlineWidth);
  }
  ...
  /// One <see cref="GameObject"/> will be returned for each part of the given area.
  /// </para></returns>
  public static GameObject[] AddAreaExternalOutline(GameObject areaObject,
      Material extrusionMaterial, Area area, float outlineWidth) {
    return ExtrudeEdgeSequences(areaObject, extrusionMaterial,
        area.GenerateExternalBoundaryEdges(), outlineWidth);
  }
  ...
  /// </summary>
  /// <param name="areaObject">
  /// The Maps Unity SDK created <see cref="GameObject"/> to which the extruded outline should be
  /// added.
  /// </param>
  /// <param name="extrusionMaterial">
  /// The <see cref="Material"/> to apply to the outline once it has been created.
  ...
  /// </para></returns>
  private static GameObject[] ExtrudeEdgeSequences(GameObject areaObject,
      Material extrusionMaterial, List<Area.EdgeSequence> edgeSequences, float outlineWidth) {
    ...
    for(int i = 0; i < loops.Count; i++) {
      // Try to make extrusion.
      GameObject extrusionGameObject;
      if (CanMakeLoft(loops[i].Vertices.ToArray(), extrusionMaterial, crossSection, DefaultWidth,
          OutlineName, out extrusionGameObject)) {
        ...
        // If extrusion failed for any reason, print an error to this effect.
        Debug.LogErrorFormat("{0} class was not able to create an outline for area \"{1}\", "
            + "loop {2} of {3}.\nFailure was caused by there being not enough vertices to make an "
            + "outline.", typeof(Extruder), areaObject.name, i + 1, loops.Count);
      }
    }
  ...
  /// vertex. For open sequences, new vertices are added by parallel extension of the first and last
  /// edge. This means that open sequences will have flat ends, while closed sequences can generate
  /// geometry with properly mitred first and last edge vertices.
  /// </para>
  /// </remarks>
  ...
  private static List<Area.EdgeSequence> PadEdgeSequences(List<Area.EdgeSequence> edgeSequences) {
    var result = new List<Area.EdgeSequence>(edgeSequences.Count);
  ...
  /// <param name="loftName">The name that should be given to the created game object.</param>
  /// <param name="createdLoft">
  /// Created extrusion <see cref="GameObject"/>, at the origin with no parent, or null if lofting
  /// failed for any reason.
  /// </param>
  /// <returns>Whether or not loft could be created.</returns>
  private static bool CanMakeLoft(Vector2[] footprintVertices, Material extrusionMaterial,
      Vector2[] crossSection, float thickness, string loftName, out GameObject createdLoft) {
    Vector3[] vertices;
    int[] indices;
    Vector2[] uvs;
    ...
    if (CanLoft(footprintVertices, crossSection, thickness, out vertices, out indices, out uvs)) {
      var extrusion = new GameObject(loftName);
      var meshFilter = extrusion.AddComponent<MeshFilter>();
      var meshRenderer = extrusion.AddComponent<MeshRenderer>();
  ...
  /// </summary>
  /// <remarks>
  /// Lofting refers to running a shape along a path, creating a 3d volume based on where the shape
  /// has travelled. For example, lofting a circle straight upwards would give a cylinder, while
  /// lofting a circle around another, larger circle would give a donut. In both cases the volume is
  /// formed by the journey of the lofted-circle along its given path.
  /// <para>
  /// Returns 3D mesh data (vertices, triangles, indices) returned in the supplied output parameter
  /// arrays.
  /// </para></remarks>
  ...
  /// last and second vertices. For an open path the prepended ghost vertex should be the reflection
  /// of the second vertex through the first path vertex (paddedPath[0] = 2 * path[0] - path[1]),
  /// and the appended ghost vertex should be the reflection of the second to last path vertex
  /// around the last path vertex (paddedPath[last + 2] = 2 * path[last] - path[last - 1]). See
  /// <see cref="PadEdgeSequences"/> for how this is done.  This pre-padding model provides simple,
  /// unified handling of both open and closed paths, allowing lofting to work on incomplete
  /// building chunks at the edge of the loaded map region.
  ...
  /// where x and y represent x and z coordinates (i.e. 2d coordinates in a ground-plane at y = 0).
  /// </para>
  /// </param>
  ...
  /// Vector2 format, where x and y represent coordinates in a flat plane comprising just the shape
  /// to loft.
  /// </param>
  /// <param name="thickness">Thickness of loft.</param>
  ...
  private static bool CanLoft(Vector2[] paddedPath, Vector2[] crossSection, float thickness,
      out Vector3[] vertices, out int[] triangleIndices, out Vector2[] uvs) {
    ...
    // Determine the total number of vertices and triangles needed to create the lofted volume. Note
    // that the total number of triangle indices is based on the fact that each combination of
    // path-segment and cross-section segment generates a quad of two triangles, requiring 6
    ...
    // simplify the calculation of previous and next directions (see PadEdgeSequences), so we ignore
    // the first and last vertices in the following loop, only considering vertices from the
    // original path.
    for (int cornerInPath = startCorner; cornerInPath < paddedPath.Length - 1; cornerInPath++) {
      ...
      // half. For very nearly parallel edges, the next and previous directions will face in almost
      // exactly opposite directions so the sum will be degenerately small. To avoid numerical
      // issues, we simply take the right hand perpendicular of the nextEdge.
      ...
      // cuts the corner in half, such that the angle between this bisector and the previous edge is
      // the same as the angle between this bisector and the next edge). If we can determine the
      // length of this bisector, we will know exactly how far to travel along it from the current
      // corner to the new, extruded corner.
      //
      //                       Previous Edge, Extruded
      //     Extruded Corner ●────────○──────────────────────────
      //                    ╱ ╲       |
      ...
      // back along the previous edge) with the up vector to find the left hand perpendicular vector
      // of the previous direction vector as shown above. (Note that Unity uses a left handed
      // coordinate system, giving us the left hand perpendicular from the cross product)
      ...
      // E (unitE) we get the vector E. This implies that the length of this projected vector is the
      // same as the length of E, which is just the extrusion distance. This gives us:
      //   B . unitE = |E|
      ...
      float distanceToExtrude = thickness
          / Vector3.Dot(normalizedExtrusionVector, rightBisector);
      ...
      // Create a copy of the cross-section shape using a coordinate system where the x-axis is the
      // rightBisector and the y-axis is the up vector. This aligns the shape to the plane
      // bisecting the joint angle in the path. When all these cross-sections are joined, they will
      // form a loft of the given cross-section around the given shape.
      for (int pointInCrossSection = 0; pointInCrossSection < crossSection.Length;
        ...
        // multiply the x values by the extrusion distance, to make sure that the loft is stretched
        // to the desired extrusion distance away from all sides of the shape.
        Vector3 pv =
            crossSection[pointInCrossSection].x * rightBisector * distanceToExtrude;
        ...
        // Copy vertices so can have un-smoothed normals. In order to have normals be unique to each
        // face, each corner must have multiple vertices, one for each face.
  ...
  /// <remarks>
  /// This function ensures that returned modulated value will always be positive for values greater
  /// than the negative of the modulus argument.
  /// </remarks>
