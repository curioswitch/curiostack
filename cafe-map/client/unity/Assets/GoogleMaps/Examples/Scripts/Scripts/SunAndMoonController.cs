...
#region Parameters
  [Tooltip("Current value of light, where 0f is midday, 0.5f is midnight, and 1f is midday again.")]
  ...
  [Range(0f, 12f), Tooltip("Total hours after sunrise and before sunset when light as at its most "
      + "vivid color (e.g. 1f means light will be fully colored as sunset 1 hour before sun fully "
      + "sets and night begins.")]
  public float VividToDark = 1f;

  [Tooltip("Size of the sun in Skybox Material (default value for this Procedural Skybox Shader is "
  ...
  [Tooltip("Render distance of Camera.main - extended so ground plane will merge seamlessly into"
      + "skybox.")]
  ...
  [Tooltip("Script for setting emission values on given Materials, used to simulate building "
...
#region Variables
  ...
  /// <summary>
  /// Azimuth of the sun/moon (what direction the sun/moon is parallel to the ground, as opposed to
  /// zenith which is how high the sun/moon is relative to the horizon).
  /// </summary>
  ...
  /// <summary>
  /// Calculated midpoint between sunset and sunrise that is always small (below 0f or near 0f), for
  /// use in calculating percents between full moon and start of sunrise.
  /// </summary>
  ...
  /// <summary>
  /// Color of ambient light at sunrise and sunset - taken from given Half Light Color multiplied by
  /// Ambient Day Percent.
  /// </summary>
...
#region Input
  ...
  internal void Set(float newValue) {
    ...
    if (newValue < 0f || newValue > 1f) {
      ...
      Debug.LogWarningFormat("Invalid value of {0} given to {1}.{2}.Set, which expected a value "
          + "between 0f and 1f.\nAdjusting value from {0} to {3}.",
          originalValue, name, GetType(), newValue);
    }
    ...
    if (FullMoonAfterMidnight && newValue < FullMoon) {
      ...
      // Set up values for - Sunset to Midnight.
      // Note that these values may be set up slightly differently depending on whether full moon is
      // nearer to 0f or 1f.
      if (FullMoonAfterMidnight) {
    ...
    CurrentEmission = emissionStatic
        ?? Mathf.Lerp(emissionStart.Value, emissionEnd.Value, CurrentValue);
    EmissionController.SetEmission(CurrentEmission);

    // Set the current ambient light.
    CurrentAmbient = ambientStatic
        ?? Color.Lerp(ambientStart.Value, ambientEnd.Value, CurrentValue);
    ...
    Color fogColor = fogStatic
        ?? Color.Lerp(fogStart.Value, fogEnd.Value, CurrentValue);
    RenderSettings.fogColor = fogColor;
    Sky.SetColor("_GroundColor", fogColor);

    // Color sky with day/night transition.
    Sky.SetFloat("_AtmosphereThickness", thicknessStatic
        ?? Mathf.Lerp(thicknessStart.Value, thicknessEnd.Value, CurrentValue));
  }
#endregion
...
#region Private
  ...
  private static float Smoothed (float value, bool smoothStart, bool smoothEnd) {
    if (smoothStart) {
      return smoothEnd
          ? (Mathf.Sin ((value - 0.5f) * Mathf.PI) + 1f) / 2f
