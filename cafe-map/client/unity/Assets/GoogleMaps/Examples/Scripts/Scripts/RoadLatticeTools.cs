...
public class RoadLatticeTools {
  /// <summary>
  /// The maximium number of segments a lattice representation can contain before it will be split
  /// into multiple submeshes to avoid unity mesh vertex limit issues.
  /// </summary>
  private static int SubmeshSegmentLimit = 2000;

  /// <summary>
  /// Maximum size of the gap in the middle of the rectangular geometry representing a road segment.
  /// </summary>
  ...
  /// <summary>
  /// Instantiate a number of debug meshes from the supplied list of road lattice segments, dividing
  ...
  private static void MakeSublatticeMeshes(GameObject parent,
      List<Vector2> segments, Material material) {
    ...
    for (int start = 0; start < segments.Count; start += SubmeshSegmentLimit) {
      ...
      for (int i = start; i < segments.Count && i < start + SubmeshSegmentLimit; i += 2) {
        ...
        // Create two quads starting/ending either side of the midpoint, leaving the calculated gap.
        Vector2 v1Neighbour = midpoint - direction * delta;
        Vector2 v2Neighbour = midpoint + direction * delta;
        AddLineSegmentQuad(v1, v1Neighbour, 1, vertices, triangles);
        AddLineSegmentQuad(v2, v2Neighbour, 1, vertices, triangles);
      }
  ...
  public static GameObject MakePathDebugObject(
    ...
    for (int i = 0; i < nodeLocations.Count - 1; i++) {
      AddLineSegmentQuad(nodeLocations[i], nodeLocations[i + 1], width, vertices, triangles);
    }
  ...
  /// line segment between the supplied ground plane 2-dimensional points, using the supplied width.
  /// </summary>
  /// <remarks>
  /// Four vertices are added to the vertices list, and the six indices for two triangles are added
  ...
  private static void AddLineSegmentQuad(
      Vector2 p1, Vector2 p2, float width, List<Vector3> vertices, List<int> triangles) {
