...
public sealed class EmissionController : MonoBehaviour {
  [Tooltip("Start/current emission value, in the range from 0f to 1f, where 0f means no emission "
      + "and 1f means full emission.")]
  public float Value = 0.5f;

  [Tooltip("Materials to control emission of. Skipped if no Materials given, and error printed if "
  ...
  [Tooltip("Minimum emission color to apply when emission is 100%. Emission colors are created "
  ...
  [Tooltip("Maximum emission color to apply when emission is 100%. Emission colors are created "
      + "randomly for each Material within the range of this minimum and maximum emission value to "
      + "give more natural randomness to the world.")]
  ...
  internal void SetEmission(float emissionValue) {
    ...
    if (!HasMaterials) {
      ...
      Debug.LogWarningFormat("Value of {0} given as {1}.{2}.Set, but {1} is controlling no "
          + "Materials, as none were defined as {1}.{2}.Materials.",
          emissionValue, name, GetType());
      return;
    }
    ...
    if (emissionValue < 0f || emissionValue > 1f) {
      ...
      Debug.LogWarningFormat("Invalid value of {0} given as {1}.{2}.Value, which expected a value "
          + "in the range 0 to 1 (representing 0% to 100% emission)\n  Clamping to {3}.",
          originalValue, name, GetType(), emissionValue);
    }
  ...
  private bool VerifyParameters() {
    ...
    if (string.IsNullOrEmpty(EmissionVariable)) {
      Debug.LogErrorFormat("No variable name given as {0}.{1}.Emission Variable, which is needed "
    ...
    if (Value < 0f || Value > 1f) {
      ...
      Debug.LogWarningFormat("Invalid starting value of {0} given as {1}.{2}.Value, which expected "
          + "a value in the range 0 to 1 (representing 0% to 100% emission).\nClamping to {3}.",
          originalValue, name, GetType(), Value);
    }
    ...
    // SetMaterials function later. The Materials array is ony checked here if an array of starting
    // Materials was defined as a parameter.
  ...
  /// null <see cref="Material"/>), returning false if not.
  /// </summary>
