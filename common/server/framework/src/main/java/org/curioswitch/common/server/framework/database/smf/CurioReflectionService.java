 ...
 *
 * Most of the content is the same as DefaultReflectionService,
 * with the change to use org.ow2.asm instead of org.simpleflatmapper.ow2ams.
 */
public class CurioReflectionService extends ReflectionService {
    ...
    private final ConcurrentMap<String,  UnaryFactory<Type, Member>> builderMethods = new ConcurrentHashMap<>();
    ...
    private CurioReflectionService(boolean isAsmActivated,
    ...
    private static List<ExecutableInstantiatorDefinition> kotlingDefaultValueConstructor(List<InstantiatorDefinition> list) {
    ...
    private static boolean isKotlinOriginalConstructor(ExecutableInstantiatorDefinition def, InstantiatorDefinition id) {
        if (id instanceof ExecutableInstantiatorDefinition) {
    ...
    private static Consumer<BiConsumer<String, UnaryFactory<Type, Member>>>[] getPredifinedBuilderProducers() {
        ...
        consumer.accept(biConsumer -> biConsumer.accept("javax.money.MonetaryAmount",
    ...
    private static final AsmFactory _defaultAsmFactory = new AsmFactory(CurioReflectionService.class.getClassLoader());
    ...
    public static CurioReflectionService newInstance(boolean useAsmGeneration) {
        return new CurioReflectionService(useAsmGeneration && canSeeSetterFromContextClassLoader() ? _defaultAsmFactory : null);
    }
