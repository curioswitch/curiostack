...
export JAVA_HOME="$OPENJDK_DIR/jdk-zulu14.29.23-ca-jdk14.0.2"

DEST="$OPENJDK_DIR/jdk-zulu14.29.23-ca-jdk14.0.2.tar.gz.or.zip"

if "$linux" = "true"; then
  SRC="https://cdn.azul.com/zulu/bin/zulu14.29.23-ca-jdk14.0.2-linux_x64.tar.gz"
fi

if "$darwin" = "true"; then
  SRC="https://cdn.azul.com/zulu/bin/zulu14.29.23-ca-jdk14.0.2-macosx_x64.tar.gz"
  export JAVA_HOME="$JAVA_HOME/Contents/Home"
fi

if "$windows" = "true"; then
  SRC="https://cdn.azul.com/zulu/bin/zulu14.29.23-ca-jdk14.0.2-win_x64.zip"
fi
...
if [ ! -d "$JAVA_HOME" ]; then
  ...
  if "$windows" = "true"; then
    ...
    mv "${OPENJDK_DIR}/zulu14.29.23-ca-jdk14.0.2-win_x64" "$JAVA_HOME"
  else
    mkdir -p "$JAVA_HOME"
    if "$darwin" = "true"; then
